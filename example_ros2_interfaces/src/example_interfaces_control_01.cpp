...
class ExampleInterfacesContro: public rclcpp::Node
{
    public:
    ExampleInterfacesContro(std::string name):Node(name){
        ...
        //订阅机器人状态话题
        robot_status_subsribe_ = this->create_subscription<example_ros2_interfaces::msg::RobotStatus>("robot_status",10,std::bind(&ExampleInterfacesContro::robot_status_callback_,this,std::placeholers::_1));
  ...
  void move_robot(float distance){
    ...
    while(!client_->wait_for_service(std::chrono::seconds(1))){
        //等待时检测rclcpp的状态
        if(!rclcpp::ok()){
            RCLCPP_ERROR(this->get_logger(),"等待服务的过程被中断");
            return;
        }
    ...
    client_ ->async_send_request(
        request,std::bind(&ExampleInterfacesContro::result_callback_,this,std::placeholders::_1);
    ...
    rclcpp::Subscription<example_ros2_interfaces::msg::RobotStatus>::SharedPtr robot_status_subsribe_;
    //机器人移动结果回调函数
    void result_callback_(rclcpp::Client<example_ros2_interfaces::srv::MoveRobot>::SharedFuture result_future){
        auto response = result_future.get();
        RCLCPP_INFO(this->get_logger(),"移动结果：%f",response->pose);
